...
entity Canale_secondi is
    Port ( 
        ...
        ratemeter    : in Freq_array;
        COUNTER200M  : in STD_LOGIC_VECTOR(11 downto 0);
        ...
        dead_time    : out STD_LOGIC_VECTOR(31 downto 0);
        wr_en 	     : out STD_LOGIC
        );
end Canale_secondi;
...
----DEADTIME-----
signal dead_time_s  : std_logic_vector(31 downto 0);
signal dead_counter : std_logic_vector(31 downto 0);
----CRC----------
...
signal partial0_3   : std_logic_vector(26 downto 0):=(OTHERS => '0');
signal partial4_7   : std_logic_vector(26 downto 0):=(OTHERS => '0');
signal partial8_11  : std_logic_vector(26 downto 0):=(OTHERS => '0');
signal partial12_15 : std_logic_vector(26 downto 0):=(OTHERS => '0');
signal partial16_18 : std_logic_vector(26 downto 0):=(OTHERS => '0');
signal partial_a    : std_logic_vector(28 downto 0):=(OTHERS => '0');
signal partial_b    : std_logic_vector(28 downto 0):=(OTHERS => '0');
signal Shifter      : std_logic_vector(5 downto 0):=(OTHERS => '0');
signal Flag_int     : std_logic;
signal sum_done     : std_logic;
signal Flg_clean    : std_logic;
signal rate_sum     : std_logic_vector(29 downto 0):=(OTHERS => '0');
----FSM----------
...
begin
	if reset = '1' then
        ...
        rate_sum     <= ('0' & partial_a)      + ('0' & partial_b);
...
FSM_state_decode: process(present_state, pps_r, wr_ack, sum_done, HF_FIFO, unix_reg, extclk0status, extclk1status, rate_sum, PLL_locked, pow_cons_ok, voltage_ok, unix_error, humidity, temperature, dead_time_s, CRC)
begin
    ...
    when ROW_2 =>
        Flg_clean      <= '0';
        wr_en          <= '1';
        word_to_fifo_s <= "00" & unix_reg(31 downto 26) & rate_sum(23 downto 0); -- 00+unixtime(6)+ratemeter(24)
        if wr_ack = '1' then
            wr_en      <= '0';
            next_State <= ROW_3;
        else
   ...
   when CRC_s =>
        Flg_clean      <= '0';
        wr_en          <= '0';
        word_to_fifo_s <= "11" & "0000" & "1111111111" & dead_time_s(15 downto 0); -- 11+CRC(4)+stateCH(11)+dead_time(16)
        next_state     <= ROW_4; 
    ...
    when ROW_4 =>
        Flg_clean      <= '0';
        wr_en          <= '1';
        word_to_fifo_s <= "11" & CRC & "1111111111" & dead_time_s(15 downto 0); -- 11+CRC(4)+stateCH(11)+dead_time(16)
        if wr_ack = '1' then
            wr_en          <= '0';
            next_state     <= IDLE; 
...
begin
	if reset = '1' then
        ...
        if pps = '1' then
            ...
            dead_counter <= (OTHERS => '0');
        elsif (COUNTER200M(11 downto 0) = 0 ) and (dead_counter < X"FFFFFFFF") and (HF_FIFO = '0') then
