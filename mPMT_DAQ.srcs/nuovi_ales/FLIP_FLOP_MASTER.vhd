...
--   )
--   port map (
--      O => q_carry(0),     -- 1-bit output: Clock output port
--      CE => '1',   -- 1-bit input: Active high, clock enable (Divided modes only)
--      CLR => '0', -- 1-bit input: Active high, asynchronous clear (Divided modes only)
--      I => d      -- 1-bit input: Clock buffer input driven by an IBUF, MMCM or local interconnect
--   );
...
clocked1 : process(Multiclk(1),RESET)
begin
	if(RESET='1')then
		q_intermedio_1(0)<='0';
		q_intermedio_1(7)<='0';
	...
	elsif(rising_edge(Multiclk(1)))then
...
clocked2 : process(Multiclk(2),RESET)
begin
	if(RESET='1')then
		q_intermedio_1(1)<='0';
	elsif(rising_edge(Multiclk(2)))then
		q_intermedio_1(1)<=q_intermedio_0(1);
	end if;
end process;

clocked3 : process(Multiclk(3),RESET)
begin
	if(RESET='1')then
		q_intermedio_1(2)<='0';
		q_intermedio_1(9)<='0';
		q_intermedio_1(8)<='0';
	elsif(rising_edge(Multiclk(3)))then
...
clocked4 : process(Multiclk(4),RESET)
begin
	if(RESET='1')then
		q_intermedio_1(3)<='0';
		q_intermedio_1(10)<='0';
	...
	elsif(rising_edge(Multiclk(4)))then
...
clocked5 : process(Multiclk(5),RESET)
begin
	if(RESET='1')then
		q_intermedio_1(4)<='0';
		q_intermedio_1(11)<='0';
	elsif(rising_edge(Multiclk(5)))then
		q_intermedio_1(4)<=q_intermedio_0(4);
		q_intermedio_1(11)<=q_intermedio_0(11);
	end if;
end process;

clocked6 : process(Multiclk(6),RESET)
begin
	if(RESET='1')then
		q_intermedio_1(5)<='0';
		q_intermedio_1(12)<='0';
	...
	elsif(rising_edge(Multiclk(6)))then
...
clocked7 : process(Multiclk(7),RESET)
begin
	if(RESET='1')then
		q_intermedio_1(6)<='0';
		q_intermedio_1(14 downto 13)<="00";
	elsif(rising_edge(Multiclk(7)))then
